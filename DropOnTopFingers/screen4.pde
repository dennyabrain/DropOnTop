void screen4(){
 takeTest=loadImage("Image/takeTest.png");//To get rid of the crash
 if(isAnimating){
   check.resize(radiusButton*2,radiusButton*2);
 } 
 image(check,width-140,height-140);
  ...
  float cur_size = cursor_size*scale_factor; 
   ...
   if (a!=null) {
     ...
     if (a.x>0 && a.x<width && a.y>0 && a.y<height) {
       ...
       fill(245,206,77);
       textSize(20);
       measuredAngle=int(abs(DotObjects[0].angle-DotObjects[1].angle));
       text(""+measuredAngle, a.x, a.y-100);
     }
   }
   else{
